...
## 2026-08-30 - 网表归档清理与中文说明统一

### 变更

- 当前 v2.3.0 仅使用 `Netlist_474控制板_2026-08-28.net` 和 `Netlist_非同步功率板_2026-08-28.net`。
- 删除未参与当前硬件配置和构建检查的历史文件：旧版功率板网表、同步半桥小板网表。
- 删除检查脚本中对同步半桥历史网表的强制校验，保留控制板与非同步功率板的引脚、ADC、PWM 和跨板网络校验。
- 根目录 `README.md` 的 REMADE/工程说明改为中文，明确活动网表和维护范围，避免把历史网表误认为当前硬件。

### 依据与风险

删除的两个文件仅用于历史审查，不属于当前外部保护、CHE2/CHF1 和 ADC 路由；如后续重新使用对应硬件，必须以新日期网表重新审查并单独提交。

## 2026-08-30 - 接手文档中文化与维护规则补全

- `[STATIC]` 重写根目录 `README.md`，改为中文并补充克隆、阅读日志、修改范围、无窗口检查/编译/烧录、串口验证、安全边界和提交要求。
- `[STATIC]` 重写 `docs/MAINTENANCE.md`，明确真值源、每次变更流程、日志字段、证据等级、生成物禁提交规则和 Git 推送规则。
- `[STATIC]` 保留命令、路径、芯片型号和证据标签的原文形式，避免接手者复制命令时产生歧义。
